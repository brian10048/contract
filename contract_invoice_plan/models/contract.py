...
class ContractContract(models.Model):
    ...
    contract_line_ip_ids = fields.One2many(
        string="Contract lines (IP)",
    ...
    def _compute_amount_total(self):
        for rec in self:
            rec.amount_total = sum(rec.contract_line_ip_ids.mapped("price_subtotal"))
...
class ContractInvoicePlan(models.Model):
    ...
    def _compute_to_invoice(self):
        ...
        for rec in self.sorted("installment"):
            # Not draft, to_invoice = False
            # if rec.contract_id.state != "purchase":
            #     continue
            if not rec.invoiced:
                rec.to_invoice = True
                break
    ...
    def _update_new_quantity(self, line, percent):
        """Hook function"""
        plan_qty = line.quantity * (percent / 100)
